...
def list_entries(url: str) -> List[PlaylistEntry]:
    ...
    """
    opts: Dict[str, Any] = {**_COMMON_OPTS, "extract_flat": "in_playlist"}
...
def playlist_title(url: str) -> Optional[str]:
    ...
    """
    opts: Dict[str, Any] = {**_COMMON_OPTS, "extract_flat": "in_playlist", "playlist_items": "1"}
    info: Optional[Dict[str, Any]] = _extract(opts, url)
    if info is None:
        return None
...
def fetch_metadata(video_url: str) -> Dict[str, Any]:
    ...
    """
    info: Optional[Dict[str, Any]] = _extract(dict(_COMMON_OPTS), video_url)
    if info is None:
        raise RuntimeError(f"Failed to fetch metadata for {video_url}")
    return info


def download_wav(video_url: str, audio_dir: Path, file_stem: str) -> Path:
    """Download best-quality audio and convert it to WAV.

    Downloads ``bestaudio`` and runs the ffmpeg post-processor to produce a
    ``.wav`` file named ``<file_stem>.wav`` inside ``audio_dir``.
